...
## remove deleted CAMs
t2_questionnaireCAMs <- t2_questionnaireCAMs[t2_questionnaireCAMs$PROLIFIC_PID %in% CAM_nodes_clean$participantCAM,]

## one person with missing data post CAMs
# colSums(is.na(t2_questionnaireCAMs))
t2_questionnaireCAMs[t2_questionnaireCAMs$PROLIFIC_PID == "63cedc8a97a0bb604736baea",]
t2_questionnaireCAMs <- t2_questionnaireCAMs[t2_questionnaireCAMs$PROLIFIC_PID != "63cedc8a97a0bb604736baea",]
```
...
## one person with missing data post CAMs
CAMfiles[[1]] <- CAMfiles[[1]][!CAMfiles[[1]]$participantCAM %in% "63cedc8a97a0bb604736baea", ]
CAMfiles[[2]] <- CAMfiles[[2]][!CAMfiles[[2]]$participantCAM %in% "63cedc8a97a0bb604736baea", ]
CAMfiles[[3]] <- CAMfiles[[3]][!CAMfiles[[3]]$participantCAM.x %in% "63cedc8a97a0bb604736baea", ]



CAM_nodes_clean <- CAM_nodes_clean[!CAM_nodes_clean$participantCAM %in% "63cedc8a97a0bb604736baea", ]
...
## clean data manually
CAMfiles[[1]]$text_summarized <- str_trim(string = CAMfiles[[1]]$text_summarized, side = "both")

str_subset(string = CAMfiles[[1]]$text_summarized, pattern = " _")
...
CAMfiles[[1]]$text_summarized[CAMfiles[[1]]$text_summarized == "environmental damaging behavior_negative" | CAMfiles[[1]]$text_summarized == "environmentally damaging behavior_negative"] <- "environmental damaging behavior_negative"
...
# colnames(t2_questionnaireCAMs[, c(48:79)])
...
if (all(t2_questionnaire$PROLIFIC_PID == networkIndicators$participantCAM)) {
  ...
  t2_questionnaireCAMs <-
    cbind(t2_questionnaire, networkIndicators)
  
  
  
  ## add duration CAM
  t2_questionnaireCAMs$CAM_duration <- NA
  
  h = 1
  for (c in t2_questionnaireCAMs$participantCAM) {
    # print(c)
    
    tmp_diffConcepts <-
      CAMfiles[[1]][CAMfiles[[1]]$participantCAM == c, "dateConceptCreated"] -
      CAMfiles[[1]][CAMfiles[[1]]$participantCAM == c, "dateCAMcreated"]
    tmp_diffConnectors <-
      CAMfiles[[2]][CAMfiles[[2]]$participantCAM == c, "dateConnectorCreated"] -
      CAMfiles[[2]][CAMfiles[[2]]$participantCAM == c, "dateCAMcreated"]
    
    t2_questionnaireCAMs$CAM_duration[h] <-  max(c(tmp_diffConcepts, tmp_diffConnectors))
    h = h + 1
  }
  
  ## in minutes
  t2_questionnaireCAMs$CAM_duration <- t2_questionnaireCAMs$CAM_duration / 60
...
if (all(t2_questionnaireCAMs$PROLIFIC_PID %in% t1_questionnaire$PROLIFIC_PID)) {
  ...
  t12_questionnaireCAMs <-
    left_join(x = t1_questionnaire, y = t2_questionnaireCAMs, by = "PROLIFIC_PID")
}
```
...
check correlations of durations

```{r}
plot(t12_questionnaireCAMs$total_min_prolific_t2, t12_questionnaireCAMs$CAM_duration)
cor(t12_questionnaireCAMs$total_min_prolific_t2, t12_questionnaireCAMs$CAM_duration, use = "pairwise")
```
