...
for(i in 1:nrow(applyApproximate)){
  ...
  dist <- stringdist::stringdist(a = applyApproximate$originalWord[i], applyApproximate$originalWord)


  
if(!all(unique(applyApproximate$originalWord[dist <= defineDistance]) == applyApproximate$originalWord[i])){
  
  cat("\n if you want to summarize the followings words:\n")
  print(unique(applyApproximate$originalWord[dist <= defineDistance]))
  ...
  if(out != "c"){
      applyApproximate$summarizedWord[applyApproximate$summarizedWord %in% unique(applyApproximate$originalWord[dist <= defineDistance])] <- out
  }
}
}
