...
```{r}
AIT_coded$valence <- as.numeric(AIT_coded$valence)
AIT_coded$valence[is.na(AIT_coded$valence)] <- 4 # !!!


freq_terms = AIT_coded %>% dplyr::select(codedWords) %>% dplyr::count(codedWords)
...
for(i in 1:nrow(freq_terms)){
  freq_terms$mean_valence[i] <- mean(AIT_coded$valence[AIT_coded$codedWords == freq_terms$Words[i]])
  freq_terms$sd_valence[i] <- sd(AIT_coded$valence[AIT_coded$codedWords == freq_terms$Words[i]])
}
