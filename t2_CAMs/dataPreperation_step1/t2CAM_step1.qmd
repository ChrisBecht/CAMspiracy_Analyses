...
prolific_Germany <- read.csv(file = "prolific_export_6575915db9c7c52953c7f84e.csv", header = TRUE)
prolific_USA <- read.csv(file = "prolific_export_6575932415456e65f906d790.csv", header = TRUE)

prolific <- rbind(prolific_Germany, prolific_USA)

rm(prolific_Germany); rm(prolific_USA)
setwd("..")
...
prolific_tmp <- prolific[prolific$Participant.id %in% questionnaire_Germany$PROLIFIC_PID,]
prolific_tmp <- prolific_tmp %>%
  arrange(sapply(Participant.id, function(y) which(y == questionnaire_Germany$PROLIFIC_PID)))


if(nrow(prolific_tmp) == nrow(questionnaire_Germany)){
  print("prolific_tmp data sucessfully added")
  
  questionnaire_Germany$total_min_prolific_t2 <- prolific_tmp$Time.taken / 60
  ## all time outs to NA
  questionnaire_Germany$total_min_prolific_t2[questionnaire_Germany$total_min_prolific_t2 > 3000] <- NA
}
```

#### get reaction times for single components post Germany

```{r}
dat_duration <- data.frame(duration = NA, sender = NA, ID = NA)
dat <- dat_postCAM_Germany
...
## save as .xlsx
write.xlsx2(x = dat_duration, file = "outputs/para_duration_singleComponents_Germany.xlsx")

#### plot
dat_duration$ID <- factor(dat_duration$ID)
p <- dat_duration %>%
  ggplot(aes(x=sender, y=duration, color=ID)) +
  geom_point() +
...
## save ggplot as PDF
ggsave(filename = "outputs/durations_components_Germany.pdf", p)
```
...
prolific_tmp <- prolific[prolific$Participant.id %in% questionnaire_USA$PROLIFIC_PID,]
prolific_tmp <- prolific_tmp %>%
  arrange(sapply(Participant.id, function(y) which(y == questionnaire_USA$PROLIFIC_PID)))


# questionnaire_USA[!questionnaire_USA$PROLIFIC_PID %in% prolific_tmp$Participant.id, ]

questionnaire_USA$total_min_prolific_t2 <- NA
questionnaire_USA$total_min_prolific_t2[questionnaire_USA$PROLIFIC_PID %in% prolific_tmp$Participant.id] <- prolific_tmp$Time.taken / 60
questionnaire_USA$total_min_prolific_t2[questionnaire_USA$total_min_prolific_t2 > 3000] <- NA
```
...
## save as .xlsx
write.xlsx2(x = dat_duration, file = "outputs/para_duration_singleComponents_USA.xlsx")
...
## save ggplot as PDF
ggsave(filename = "outputs/durations_components_USA.pdf", p)
```
...
colnames(questionnaire_CAM_Germany)[76:79] <- colnames(questionnaire_CAM_USA)[76:79]
